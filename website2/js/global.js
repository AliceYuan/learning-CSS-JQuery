$(document).ready(function(){
  statusBarCompletion();
  $('.dropDownIcon').mousedown(function() {
    if ($(this).hasClass('up')){
      $(this).removeClass("up");
      $(this).addClass("down");
...
function statusBarCompletion() {
  $('.dropDown li').each(function(i) {
    console.log(this);
    var $completionObj = $(this).find('.status-bar .completion');
    //find status-bar completion class
    var $className = $($completionObj).attr('class'); //get class name
    var $completionPer = $className.replace('completion ','');
    //remove completion string from class name
    $($completionObj).css('width', $completionPer+'%');
    //style width to be % found from class name
  });
